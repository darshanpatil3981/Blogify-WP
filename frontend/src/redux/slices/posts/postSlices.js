  ...
  async (post, { rejectWithValue, getState, dispatch }) => {
    ...
    try {
      ...
      const { data } = await axios.post(
        `https://backendtry2022.herokuapp.com/api/posts`,
        formData,
        config
      );
      //dispatch action
  ...
  async (post, { rejectWithValue, getState, dispatch }) => {
    ...
    try {
      ...
      const { data } = await axios.put(
        `https://backendtry2022.herokuapp.com/api/posts/${post?.id}`,
        post,
        config
      );
      //dispatch
  ...
  async (postId, { rejectWithValue, getState, dispatch }) => {
    ...
    try {
      ...
      const { data } = await axios.delete(
        `https://backendtry2022.herokuapp.com/api/posts/${postId}`,
        config
      );
      //dispatch
  ...
  async (category, { rejectWithValue, getState, dispatch }) => {
    try {
      const { data } = await axios.get(
        `https://backendtry2022.herokuapp.com/api/posts?category=${category}`
      );
      return data;
    } catch (error) {
      if (!error?.response) throw error;
...
export const fetchPostDetailsAction = createAsyncThunk(
  ...
  async (id, { rejectWithValue, getState, dispatch }) => {
    try {
      const { data } = await axios.get(`https://backendtry2022.herokuapp.com/api/posts/${id}`);
      return data;
    } catch (error) {
      if (!error?.response) throw error;
      return rejectWithValue(error?.response?.data);
...
export const toggleAddLikesToPost = createAsyncThunk(
  ...
  async (postId, { rejectWithValue, getState, dispatch }) => {
    ...
    try {
      const { data } = await axios.put(
        `https://backendtry2022.herokuapp.com/api/posts/likes`,
        { postId },
        config
      );
...
export const toggleAddDisLikesToPost = createAsyncThunk(
  ...
  async (postId, { rejectWithValue, getState, dispatch }) => {
    ...
    try {
      const { data } = await axios.put(
        `https://backendtry2022.herokuapp.com/api/posts/dislikes`,
        { postId },
        config
      );
