  ...
  async (comment, { rejectWithValue, getState, dispatch }) => {
    ...
    try {
      const { data } = await axios.post(
        `https://backendtry2022.herokuapp.com/api/comments`,
        {
  ...
  async (commentId, { rejectWithValue, getState, dispatch }) => {
    ...
    try {
      const { data } = await axios.delete(
        `https://backendtry2022.herokuapp.com/api/comments/${commentId}`,
        config
      );
      return data;
    } catch (error) {
  ...
  async (comment, { rejectWithValue, getState, dispatch }) => {
    ...
    try {
      const { data } = await axios.put(
        `https://backendtry2022.herokuapp.com/api/comments/${comment?.id}`,
        { description: comment?.description },
        config
      );
      //dispatch
...
export const fetchCommentAction = createAsyncThunk(
  ...
  async (id, { rejectWithValue, getState, dispatch }) => {
    ...
    //http call
    try {
      const { data } = await axios.get(`https://backendtry2022.herokuapp.com/api/comments/${id}`, config);
      return data;
    } catch (error) {
      if (!error?.response) {
        throw error;
